...
// ========================================
// MOBILE SCROLL FIX - ADD TO YOUR app.js
// ========================================

// 1. UTILITY FUNCTION: Smooth scroll to top with mobile optimization
...
// 2. ENHANCED showTab function with scroll reset
function showTab(tabName) {
    console.log(`🔄 Switching to tab: ${tabName}`);

    // IMMEDIATE scroll to top when switching tabs
    scrollToTop(false); // Instant scroll for tab switches
    
    // Rest of your existing showTab logic...
    if (['dashboard', 'calendar', 'appliances', 'documents'].includes(tabName)) {
        document.body.classList.add('main-app-active');
        document.body.classList.remove('hide-header', 'show-header');
        console.log('✅ Header made sticky for tab:', tabName);
    ...
    // Hide ALL views
    const views = ['dashboard-view', 'calendar-view', 'documents-view', 'appliances-view'];
    ...
    console.log(`✅ Switched to ${tabName} tab and scrolled to top`);
}

// 3. ENHANCED showAllTasks with scroll reset
function showAllTasks() {
    console.log('📋 Switching to All Tasks view...');
    ...
    console.log('✅ All Tasks view displayed with scroll reset');
}

// 4. SETUP NAVIGATION FIXES
function goBackToHomeSetup() {
    scrollToTop(true); // Smooth scroll for setup navigation
    ...
    console.log('✅ Navigated back to setup with scroll reset');
}

function finishTaskSetup() {
    console.log('🚀 Starting task setup completion...');
    
    // Your existing task setup logic here...
    let successCount = 0;
    let errorCount = 0;
        ...
        if (task.isTemplate) {
            try {
                ...
                if (task.season) {
                    ...
                    switch(task.season) {
                        case 'spring': targetMonth = 2; break;
                        case 'summer': targetMonth = 5; break;
                        case 'fall': targetMonth = 8; break;
                        case 'winter': targetMonth = 10; break;
                        default: targetMonth = currentMonth;
                    }
    ...
    // CRITICAL: Scroll to top before switching to main app
    scrollToTop(false);
    ...
    if (taskSetupEl) {
        taskSetupEl.classList.add('hidden');
        taskSetupEl.style.display = 'none';
    }
    ...
    console.log('🎉 Setup completion with scroll reset successful!');
}

// 5. MODAL FIXES - Reset scroll when opening/closing modals
function openTaskEditModal(task, isNew = false) {
    ...
    // IMPORTANT: Prevent background scroll on mobile
...
function closeTaskEditModal() {
    ...
    // IMPORTANT: Restore scroll and reset position
    document.body.style.overflow = '';
    window.currentEditingTask = null;
    
    // Small delay then scroll to top
    setTimeout(() => {
        scrollToTop(true);
    }, 100);
}

// 6. ADD SCROLL RESET TO EXISTING FUNCTIONS
// Update your existing functions by adding these lines:

// In celebrateStepCompletion:
function celebrateStepCompletion(currentStep, nextStep) {
    ...
    if (progressBar) {
        const newWidth = Math.round((nextStep / 3) * 100);
        progressBar.style.width = `${newWidth}%`;
    }
    
    if (typeof updateTaskPreview === 'function') {
        updateTaskPreview();
    }
    ...
    setTimeout(() => {
        ...
        const nextStepEl = document.querySelector(`.onboarding-step[data-step="${nextStep}"]`);
        if (nextStepEl) {
            nextStepEl.classList.remove('hidden');
            
            // SCROLL TO TOP instead of scrollIntoView
            scrollToTop(true);
        }
    }, 100);
}

// 7. EXPORT UPDATED FUNCTIONS
window.showTab = showTab;
window.showAllTasks = showAllTasks;
window.goBackToHomeSetup = goBackToHomeSetup;
window.finishTaskSetup = finishTaskSetup;
window.openTaskEditModal = openTaskEditModal;
window.closeTaskEditModal = closeTaskEditModal;
window.celebrateStepCompletion = celebrateStepCompletion;
window.scrollToTop = scrollToTop;

console.log('✅ Mobile scroll optimization loaded - all screens will start from top!');
